...
class Model(nn.Module):
    ...
    def get_text_embeddings(self, texts):
        ...
        with th.no_grad():
            embeddings = self.text_encoder(input_ids)[0][:, 0, :]
        return embeddings

    def forward(self, im, bg, hard=False, custom_dict=None, rng=None, custom_bg=None, texts=None):
        bs = im.shape[0]
        ...
        if texts is not None:
            text_codes = self.get_text_embeddings(texts)
            text_codes = text_codes.unsqueeze(1).repeat(1, im_codes.shape[1], 1)
            im_codes = th.cat([im_codes, text_codes], dim=-1)
