...
class AnimationDataset(th.utils.data.Dataset):
    def __init__(self, data, canvas_size=128):
        ...
        self.image_files = [os.path.join(data, 'images', f)
        ...
        self.text_files = [os.path.join(data, 'txt', f)
                           for f in os.listdir(os.path.join(data, 'txt'))
                           if f.endswith('txt')]
        ...
        colors = []
        for f in np.random.choice(self.image_files, size=min(len(self.image_files), 100),
                                  replace=False):
